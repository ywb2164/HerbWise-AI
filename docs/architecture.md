...
V0.3A adds provider contracts, `SecretResolver`, OpenAI-compatible text/vision adapters, a lazy local vision adapter, database-backed name normalization, and deterministic fusion. Real providers are never called by default; `RAG_MODE` remains `mock`.
