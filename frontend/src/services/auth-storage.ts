...
const ADMIN_HANDOFF_PREFIX = 'HERBWISE_ADMIN_AUTH:'
...
export function getAdminPortalUrl(): string {
  return (import.meta.env.VITE_ADMIN_URL || 'http://localhost:9528').replace(/\/$/, '')
}

export function handoffToAdmin(): void {
  const accessToken = getAccessToken()
  const refreshToken = getRefreshToken()
  if (!accessToken || !refreshToken) throw new Error('管理员会话不存在，请重新登录')

  window.name = `${ADMIN_HANDOFF_PREFIX}${btoa(JSON.stringify({ accessToken, refreshToken }))}`
  window.location.assign(`${getAdminPortalUrl()}/home`)
}
