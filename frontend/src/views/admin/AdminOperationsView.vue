...
const taskColumns: DataTableColumns<AdminTaskRecord> = [
  { key: 'task_id', title: '任务 ID', width: 210, ellipsis: { tooltip: true } },
...
const logColumns: DataTableColumns<AdminAgentLog> = [
  { key: 'task_id', title: '任务 ID', width: 200, ellipsis: { tooltip: true } },
  ...
  { key: 'elapsed_ms', title: '耗时', width: 100, render: (row) => formatDuration(row.elapsed_ms) },
  {
    ...
    render: (row) => row.output_summary === 'mock node completed' ? '规则节点执行完成' : row.output_summary || '--',
  },
