...
function renderCell(row: AdminRecord, key: string, kind = 'text') {
  ...
  if (key === 'model_name' && value === 'mock-llm') return '规则引擎'
  if (key === 'config_code' && value === 'mock-default') return 'builtin-rules'
  if (kind === 'boolean') {
    return h(
      NTag,
  ...
  if (kind === 'code') return h('code', { class: 'admin-code-cell' }, String(value ?? '--'))
  return String(value ?? '--')
}
