...
import { Pencil, Plus, RefreshCw, Trash2 } from '../../icons'
  ...
  { key: 'last_login_at', title: '最近登录', width: 170, render: (row) => formatDate(row.last_login_at) },
  {
    ...
    render: (row) => h(NSpace, { size: 2, wrap: false }, {
      default: () => [
        ...
        iconButton(
          Trash2,
          String(row.id) === auth.userInfo.userId ? '不能删除当前账号' : '删除',
          () => confirmDelete(row),
          String(row.id) === auth.userInfo.userId,
        ),
      ],
    }),
  },
...
function confirmDelete(user: AdminUser): void {
  if (String(user.id) === auth.userInfo.userId) return
  dialog.warning({
    title: '删除用户',
